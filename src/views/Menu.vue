<template>
  <div class="min-h-screen flex flex-col justify-center items-center px-4">
    ...
    <div id="play-menu" v-if="isPlayMenuDisplayed" class="flex flex-col max-w-lg w-full mb-8">
      ...
      <h2 class="text-center mb-4 text-indigo-900 text-2xl font-bold">Difficulty - {{ selectedDifficultyName }}</h2>
      <div class="mb-12">
        <button v-for="(difficulty, index) in difficulties" :key="index" @click="selectDifficulty(index)" class="bg-indigo-700 px-5 py-2 mb-2 text-2xl uppercase rounded-lg text-white w-full">{{ difficulty.name }}</button>
      </div>
